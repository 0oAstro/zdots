# ── history-aux — SQLite + JSON shell history ────────────────────
# Mirrors Zsh built-in history to sqlite3 + JSON for richer querying.
# Adapted from mattmc3/zsh_custom.
#
# Requires: sqlite3, jq
# Search: histdb [-d] [-f] [-s] [-S] [-r] [-n N] [pattern]

() {
  (( $+commands[sqlite3] && $+commands[jq] )) || return 0

  export HISTDBFILE="${XDG_DATA_HOME:-$HOME/.local/share}/zsh/zsh_history.db"
  export HISTJSFILE="${XDG_DATA_HOME:-$HOME/.local/share}/zsh/zsh_history.json"

  zmodload zsh/datetime 2>/dev/null
  typeset -gA _history_aux_state
  if [[ -n "${_history_aux_state[loaded]:-}" ]]; then return 0; fi
...
SQL
  ...
  _history_aux_json_init() {
    local f="$1"; mkdir -p "${f:h}" || return 1
    [[ -f "$f" ]] || touch "$f"
  }

  _history_aux_sqlite_insert() {
    emulate -L zsh; setopt local_options
    local db="$1" q="'"; shift; local -a vals=("$@")
